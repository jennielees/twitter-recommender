...
class Api(object):
  ...
  def Search(self, term, since_id=None, per_page=100, page=1, lang="en"):
	...
	Args:
		...
		since_id:
		 Returns only public statuses with an ID greater than (that is,
		   more recent than) the specified ID. [Optional]
		per_page:
		 number of results to return [Optional] default=100
		page:
	...
	for x in data['results']:
		...
		# temp.user = self.GetUser(urllib.quote(x['from_user']))
   	   	temp.user = User(screen_name=x['from_user'], id=x['from_user_id'], profile_image_url=x['profile_image_url'])
		results.append(temp)
  ...
  def FriendshipExists(self,user_a,user_b):
    url = 'http://api.twitter.com/1/friendships/exists.json?user_a=%s&user_b=%s' % (user_a,user_b)
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    return data 
