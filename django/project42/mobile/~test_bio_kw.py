...
class Person():
  ...
  def return_recommendations(self):
    return sorted(self.recommends.items(), key=lambda(k,v): v,reverse=True)
...
# This is slow so cache in future
...
# Check for matches
 ...
 # print r.recommend_peeps('shaherose')
  exit()


# so we have intent i.keywords
# and peeps containing people with p.bio_kws
...
# lets do some tf idf
  ...
  for p in peeps:
    for w in p.bio_kws:
      if w in freq_dist.keys():
        freq_dist[w] += 1
      else:
        freq_dist[w] = 1 
# different approach, record who's in the freq dist and make a rec every time 
# it's incremented


  for p in peeps:
    # relevance for each keyword
    relevance = 0.0
    for k in i.keywords:
      if k in p.bio_kws:
        relevance += 1
...
#then use directed edge api to do recommends :-)


# Step 2 glue into directed edge or other rec algo
